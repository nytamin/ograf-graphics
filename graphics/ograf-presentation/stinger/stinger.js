/**
 * OGraf Stinger / Bumper
 * Starts fully transparent. A single, self-contained <svg> holds both the
 * <mask> (N panels laid out left to right, rotated 30 degrees about the
 * frame's center, each starting at 0 width and growing to fill its own
 * equal-width slot with a slight overlap to avoid seams -- so, together,
 * they wipe across to cover the whole frame -- then, after only the
 * briefest possible hold, shrink back out) and the content
 * <g mask="url(#...)"> it reveals (a white plate with the OGraf logo). Each
 * panel's width is driven by a SMIL <animate> (not CSS), since browsers
 * reliably repaint <mask> content animated via SMIL but often fail to
 * repaint the masked element when the mask's content is only animated via
 * CSS. A debug mode disables the mask and shows the same panels directly,
 * painted on top, for inspection.
 */
...
const MASK_ID = "ograf-stinger-wipe-mask";
const PANEL_STAGGER_MS = 24;
const PANEL_TRANSITION_MS = 380; // grow duration, and shrink duration
const PANEL_ROTATION_DEG = 30;
const PANEL_OVERLAP = 1.5; // extra width so adjacent panels overlap slightly, hiding seams

// A single svg viewBox shared by the mask and the content it reveals, so
// coordinates line up 1:1 (a 16:9 frame in svg user units).
const VB_W = 1600;
const VB_H = 900;
const LOGO_NATIVE_W = 1818;
const LOGO_NATIVE_H = 611;
const LOGO_DISPLAY_W = VB_W * 0.32;
const LOGO_SCALE = LOGO_DISPLAY_W / LOGO_NATIVE_W;
const LOGO_DISPLAY_H = LOGO_NATIVE_H * LOGO_SCALE;
const LOGO_X = (VB_W - LOGO_DISPLAY_W) / 2;
const LOGO_Y = (VB_H - LOGO_DISPLAY_H) / 2;

// The panels are laid out (before rotation) across a padded square large
// enough that, once rotated by PANEL_ROTATION_DEG about the frame's center,
// they still fully cover the VB_W x VB_H frame, including its corners.
const PANEL_PAD = Math.ceil(Math.sqrt(VB_W * VB_W + VB_H * VB_H)) + 60;
const PANEL_PAD_X = (VB_W - PANEL_PAD) / 2;
const PANEL_PAD_Y = (VB_H - PANEL_PAD) / 2;
const PANEL_ROTATE_TRANSFORM = `rotate(${PANEL_ROTATION_DEG} ${VB_W / 2} ${VB_H / 2})`;

/**
 * Each panel's width is driven by a nested SMIL <animate> (begin="indefinite",
 * triggered from JS via beginElement()) rather than a CSS @keyframes
 * animation. Browsers reliably repaint <mask> content when a SMIL animation
 * changes it, but commonly do NOT repaint the masked element when the mask's
 * content is only animated via CSS -- which is why the previous CSS-driven
 * version never visibly animated. The hold duration is set to exactly the
 * stagger spread across all panels, so there's only the briefest instant
 * where every panel is simultaneously fully grown (full coverage) before
 * they all immediately start shrinking back out again.
 */
function buildPanelRects(panelCount, className) {
  const slotWidth = PANEL_PAD / panelCount;
  const targetWidth = slotWidth + PANEL_OVERLAP;
  const holdMs = Math.max((panelCount - 1) * PANEL_STAGGER_MS, 1);
  const totalMs = PANEL_TRANSITION_MS * 2 + holdMs;
  const growEnd = PANEL_TRANSITION_MS / totalMs;
  const holdEnd = (PANEL_TRANSITION_MS + holdMs) / totalMs;

  let rects = "";
  for (let i = 0; i < panelCount; i++) {
    rects +=
      `<rect class="${className}" x="${PANEL_PAD_X + i * slotWidth}" y="${PANEL_PAD_Y}" width="0" height="${PANEL_PAD}" fill="#fff">` +
      `<animate class="panel-anim" attributeName="width" begin="indefinite" fill="freeze" ` +
      `dur="${totalMs}ms" calcMode="spline" ` +
      `keyTimes="0;${growEnd};${holdEnd};1" ` +
      `keySplines="0.65 0 0.35 1;0 0 1 1;0.65 0 0.35 1" ` +
      `values="0;${targetWidth};${targetWidth};0"/>` +
      `</rect>`;
  }
  return rects;
}
...
/**
 * Builds one single, self-contained svg (following the classic
 * <mask>+masked-<g> pattern): a <mask> made of N panels laid out left to
 * right (then rotated 30 degrees about the frame's center) that grow to
 * fill the frame, and a content <g mask="url(#...)"> holding the white
 * plate + OGraf logo that gets revealed by it. Everything is real, live DOM
 * (not a foreign mask-image/data-URI resource). A parallel, un-masked debug
 * group mirrors the same panels so the wipe can be inspected directly when
 * `debug` is enabled.
 */
function buildSvgMarkup(panelCount) {
  return (
    `<svg class="stinger-svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${VB_W} ${VB_H}" preserveAspectRatio="xMidYMid slice">` +
    `<defs><mask id="${MASK_ID}" maskUnits="userSpaceOnUse" x="0" y="0" width="${VB_W}" height="${VB_H}">` +
    `<g transform="${PANEL_ROTATE_TRANSFORM}">` +
    buildPanelRects(panelCount, "mask-rect") +
    `</g>` +
    `</mask></defs>` +
    `<g class="content-group" mask="url(#${MASK_ID})">` +
    `<rect class="plate" x="0" y="0" width="${VB_W}" height="${VB_H}"/>` +
    `<g transform="translate(${LOGO_X} ${LOGO_Y}) scale(${LOGO_SCALE})" class="logo-group">` +
    ...
    `<g class="debug-group">` +
    `<g transform="${PANEL_ROTATE_TRANSFORM}">` +
    buildPanelRects(panelCount, "debug-rect") +
    `</g>` +
    `</g>` +
    `</svg>`
  );
...
.plate {
  fill: var(--plate-color, #ffffff);
  filter: drop-shadow(0 0 24px rgba(0, 0, 0, 0.08));
}

.logo-group {
  filter: drop-shadow(0 6px 14px rgba(10, 16, 40, 0.3));
}
...
.debug-group .debug-rect {
  fill: #ff2ea6;
  opacity: 0.85;
}

.stinger-svg.debug .debug-group {
  display: block;
}

.stinger-svg.debug .content-group {
  mask: none;
}
`;

class OgrafStinger extends HTMLElement {
  constructor() {
    super();
    ...
    template.innerHTML = buildSvgMarkup(this._state.panelCount);
    root.appendChild(template.content);
    this._svg = root.querySelector(".stinger-svg");
    this._plate = this._svg.querySelector(".plate");
  ...
  async playAction(params) {
    this._reset();
    if (params && params.skipAnimation) {
      // Nothing to show without animation: stays transparent.
      return { statusCode: 200, currentStep: 1 };
    }

    const maskAnims = this._svg.querySelectorAll(".mask-rect > animate");
    const debugAnims = this._svg.querySelectorAll(".debug-rect > animate");
    maskAnims.forEach((anim, i) => {
      this._pendingTimers.push(
        setTimeout(() => anim.beginElement(), i * PANEL_STAGGER_MS)
      );
    });
    debugAnims.forEach((anim, i) => {
      this._pendingTimers.push(
        setTimeout(() => anim.beginElement(), i * PANEL_STAGGER_MS)
      );
    });
  ...
  async updateAction(params) {
    const panelCountChanged =
      params.data &&
      params.data.panelCount !== undefined &&
      params.data.panelCount !== this._state.panelCount;
    this._updateState(params.data);
    if (panelCountChanged) this._rebuildSvg();
    return { statusCode: 200 };
  }
  ...
  _applyState() {
    this._plate.style.setProperty("--plate-color", this._state.plateColor);
    this._svg.classList.toggle("debug", !!this._state.debug);
  }

  _rebuildSvg() {
    const panelCount = Math.max(this._state.panelCount, 2);
    this._svg.outerHTML = buildSvgMarkup(panelCount);
    this._svg = this.shadowRoot.querySelector(".stinger-svg");
    this._plate = this._svg.querySelector(".plate");
    this._applyState();
  }
  ...
  _reset() {
    ...
    this._pendingTimers = [];
    // A frozen SMIL animation (fill="freeze") keeps overriding the base
    // "width" attribute even after endElement() + setAttribute(), so the
    // only reliable way to guarantee a fully transparent, from-scratch
    // state is to throw away and rebuild the whole svg.
    this._rebuildSvg();
  }
}

export default OgrafStinger;
